...
function autoTranslate(text) {
  // If the text is a key like "auth.login", try to extract the last part
  const parts = text.split('.')
  const lastPart = parts[parts.length - 1]
  
  // Try to find translation for the full text first
  if (translations[text]) {
    return translations[text]
  }
  
  // Try to find translation for the last part
