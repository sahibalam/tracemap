...
class WorkerWizardService {
  /**
   * ✅ Save wizard step - Saves to Workers table via workerService
   * Special handling for Step 1 (basics) to ensure email is updated across ALL sections
   */
  async saveStep(userId, stepNumber, data) {
    try {
      if (!userId) throw new Error('User ID is required')
      ...
      // ✅ Special handling for Step 1 (basics) - if email is being updated
      let result
      if (stepNumber === 1 && data.emailAddress) {
        console.log(`📧 Step 1 includes email update, using updateEmailAcrossAllSections`)
        // Get current basics to merge with new data
        const currentProfile = await workerService.getWorkerProfile(userId)
        const currentBasics = currentProfile.success && currentProfile.data?.basics ? currentProfile.data.basics : {}
        
        // Merge current basics with new data
        const mergedBasics = {
          ...currentBasics,
          ...data
        }
        
        // Use the new method that updates email across ALL sections
        result = await workerService.updateEmailAcrossAllSections(
          userId, 
          data.emailAddress, 
          mergedBasics
        )
      } else {
        // Normal section update
        result = await workerService.updateSection(userId, section, data)
      }
  ...
  /**
   * ✅ Save a specific section (non-wizard) with email handling
   * Used by standalone edit pages
   */
  async saveSection(userId, section, data) {
    try {
      if (!userId) throw new Error('User ID is required')
      if (!section) throw new Error('Section name is required')
      if (!data) throw new Error('Data is required')

      console.log(`💾 Saving section ${section} for user: ${userId}`)

      // ✅ If updating basics with email, use the special method
      if (section === 'basics' && data.emailAddress) {
        console.log(`📧 Section ${section} includes email update, using updateEmailAcrossAllSections`)
        const currentProfile = await workerService.getWorkerProfile(userId)
        const currentBasics = currentProfile.success && currentProfile.data?.basics ? currentProfile.data.basics : {}
        ...
        return await workerService.updateEmailAcrossAllSections(
          userId, 
          data.emailAddress, 
          mergedBasics
        )
      ...
      // Normal section update
      return await workerService.updateSection(userId, section, data)
  ...
  async uploadProfileImage(userId, file) {
    try {
      ...
      // ✅ Use updateBasics which now handles email across all sections
      await workerService.updateBasics(userId, {
        ...basics,
        profilePreview: viewUrl || fileUrl,
        profileImageKey: fileKey,
  ...
  /**
   * ✅ Fix email inconsistencies across all sections
   * @param {string} userId - Worker's user ID
   * @param {string} preferredEmail - The email to use as source of truth (optional)
  ...
  async fixEmailInconsistencies(userId, preferredEmail = null) {
    try {
      ...
      console.log(`🔧 Fixing email inconsistencies for user: ${userId}`)

      const result = await workerService.fixEmailInconsistencies(userId, preferredEmail)
  ...
  /**
   * ✅ Get email occurrences across all sections (for debugging)
   * @param {string} userId - Worker's user ID
   * @returns {Promise} Email occurrences
   */
  async getEmailOccurrences(userId) {
    try {
      if (!userId) throw new Error('User ID is required')

      return await workerService.getEmailOccurrences(userId)
