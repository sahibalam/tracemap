...
// ... (keeping your existing EXPERIENCE_LEVELS object - it's too large to reprint here)
...
export function TradeProfileEditPage() {
  ...
  const [activeTab, setActiveTab] = useState('trade')
  const [error, setError] = useState('')
  const [hasUnsavedTradeChanges, setHasUnsavedTradeChanges] = useState(false)
  ...
  const handleTradeRowChange = (rowId, field, value) => {
    setTradeRows(prev => prev.map(row => {
      if (row.id === rowId) {
        if (field === 'trade') {
          setHasUnsavedTradeChanges(true)
          return {
            ...row,
            trade: value,
            skillGroups: {},
